...
class ShiftNodes:
    ...
    def __init__(self, span=6.0, n=161, tau=TAU, info_delta=0.01):
        ...
        for key, th in (
            ...
            self.shifted[key] = (self._logp(self.grid, th), (self.grid - np.asarray(th)) / tau**2)
...
def sample():
    ...
    """
    nd = ShiftNodes(n=61)
    theta, c, n = np.array([0.05, 0.0]), 0.0, 50_000
...
def test_detection_channel_reweights(nodes):
    ...
    tilt, _, _ = scores_from_loglike(loglike(y, nodes), nodes, log_det=k * nodes.grid[:, 0])
    shift = k * SIGMA**2 / NU2
...
def test_detection_accepts_per_galaxy_array(nodes):
    ...
    b, _, _ = scores_from_loglike(loglike(y, nodes), nodes, log_det=np.tile(v[None], (len(y), 1)))
