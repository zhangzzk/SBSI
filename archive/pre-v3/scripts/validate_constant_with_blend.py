...
def _case_filtered_table(path, columns, min_case=None, max_case=None, max_rows=0):
    ...
    parts = []
    n = 0
    with ipc.open_file(pa.memory_map(os.fspath(path))) as reader:
...
def main():
    ...
    ap.add_argument("--scene-lookup", default=None,
                    help="per-(case,input_index) intrinsic scene decomposition; merges "
                         "nbr_flux_top2/nbr_flux_thirdplus for scene-conditioned flows. "
                         "Unlike the legacy crowd lookup, coverage must be exact: an unmatched "
                         "primary is an inconsistent simulation population, not an isolated object.")
    ap.add_argument("--extra-feature-lookup", default=None,
                    help="strict per-(case,input_index) lookup carrying additional flow condition "
                         "features (for example six radial shell fluxes or Eq. 17 purity)")
    ap.add_argument("--extra-feature-columns", nargs="*", default=None,
                    help="columns to merge from --extra-feature-lookup; every requested column and "
                         "every catalogue row must match exactly")
    ...
    ap.add_argument("--primary-mag-max", type=float, default=None,
                    help="optional true-primary magnitude ceiling applied before model scoring")
    ap.add_argument("--primary-re-min", type=float, default=None,
                    help="optional true-primary size floor applied before model scoring")
    ...
    if args.primary_mag_max is not None:
        df = df[df["r_input_p"].to_numpy(float) < args.primary_mag_max].reset_index(drop=True)
    if args.primary_re_min is not None:
        df = df[df["Re_input_p"].to_numpy(float) > args.primary_re_min].reset_index(drop=True)
    if args.primary_mag_max is not None or args.primary_re_min is not None:
        print(f"true-primary score domain: mag < {args.primary_mag_max}, "
              f"Re > {args.primary_re_min}; N={len(df):,}")
    ...
    # Pure-input decomposition used by V2.6.  Every rendered input galaxy is present in the lookup,
    # including zero-neighbour objects (whose log1p fluxes are genuinely zero), so zero-filling an
    # unmatched row would hide a catalogue mismatch.  Require exact key coverage instead.
    if args.scene_lookup:
        if not os.path.exists(args.scene_lookup):
            raise FileNotFoundError(f"scene lookup not found: {args.scene_lookup}")
        available = _ipc_schema_names(args.scene_lookup)
        scols = [c for c in ("nbr_flux_top2", "nbr_flux_thirdplus") if c in available]
        if scols != ["nbr_flux_top2", "nbr_flux_thirdplus"]:
            raise ValueError(
                f"scene lookup must contain nbr_flux_top2 and nbr_flux_thirdplus; "
                f"found {scols} in {args.scene_lookup}")
        sf = _case_filtered_table(
            args.scene_lookup, ["case", "input_index", *scols],
            min_case=args.min_case, max_case=args.max_case,
        ).to_pandas()
        if sf.duplicated(["case", "input_index"]).any():
            raise ValueError(f"scene lookup has duplicate (case,input_index) keys: {args.scene_lookup}")
        sm = df[["case", "input_index"]].merge(
            sf, on=["case", "input_index"], how="left", validate="many_to_one")
        matched = sm[scols].notna().all(axis=1)
        if not bool(matched.all()):
            raise ValueError(
                f"scene lookup coverage is not exact: matched {int(matched.sum()):,}/{len(sm):,} "
                f"rows ({matched.mean():.6%})")
        for sc in scols:
            df[sc] = sm[sc].to_numpy(float)
        print(f"scene lookup: matched 100.0000% of rows; cols={scols}")

    if args.extra_feature_lookup:
        if not args.extra_feature_columns:
            raise ValueError("--extra-feature-lookup requires --extra-feature-columns")
        if not os.path.exists(args.extra_feature_lookup):
            raise FileNotFoundError(f"extra feature lookup not found: {args.extra_feature_lookup}")
        available = _ipc_schema_names(args.extra_feature_lookup)
        missing = [c for c in args.extra_feature_columns if c not in available]
        if missing:
            raise ValueError(f"extra feature lookup is missing requested columns {missing}")
        xf = _case_filtered_table(
            args.extra_feature_lookup,
            ["case", "input_index", *args.extra_feature_columns],
            min_case=args.min_case, max_case=args.max_case,
        ).to_pandas()
        if xf.duplicated(["case", "input_index"]).any():
            raise ValueError(
                f"extra feature lookup has duplicate (case,input_index) keys: "
                f"{args.extra_feature_lookup}")
        xm = df[["case", "input_index"]].merge(
            xf, on=["case", "input_index"], how="left", validate="many_to_one")
        matched = xm[args.extra_feature_columns].notna().all(axis=1)
        if not bool(matched.all()):
            raise ValueError(
                f"extra feature lookup coverage is not exact: matched "
                f"{int(matched.sum()):,}/{len(xm):,} rows ({matched.mean():.6%})")
        values = xm[args.extra_feature_columns].to_numpy(float)
        if not np.isfinite(values).all():
            raise ValueError("extra feature lookup contains non-finite values")
        for column in args.extra_feature_columns:
            df[column] = xm[column].to_numpy(float)
        print(f"extra feature lookup: matched 100.0000%; "
              f"cols={args.extra_feature_columns}")
