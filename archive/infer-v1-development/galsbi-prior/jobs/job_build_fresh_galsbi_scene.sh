...
# Generate randomized cases from the fresh GalSBI base and build the scene store.

#SBATCH --job-name=sbsi_galsbi_scene
...
root=/project/ls-gruen/users/zekang.zhang/sbsi/catalogue_prior/fresh_galsbi_v1
...
if [[ -e "$root/cases/gals0_0.0.feather" || -e "$root/scene_cases0_9.parquet" || -e "$root/scene_store" ]]; then
  echo "refusing to overwrite an existing fresh GalSBI case or scene artifact" >&2
  exit 2
fi

"$python" "$blendsim/scripts/run_pipeline.py" \
  --config "$repo/configs/catalogue_prior_fresh_galsbi_v1.yaml" --steps 1

"$python" "$repo/scripts/assemble_galsbi_scene_catalogue.py" \
  --cases-directory "$root/cases" --case-start 0 --n-cases 10 \
  --shear-label 0.0 --base-manifest "${base_prefix}_manifest.json" \
  --output "$root/scene_cases0_9.parquet" --report "$root/scene_cases0_9_report.json" \
  --primary-mag-min 18 --primary-mag-max 25.8 \
  --primary-re-min 0.5 --primary-re-max 1.5

"$python" "$repo/scripts/build_scene_prior.py" \
  --catalogue "$root/scene_cases0_9.parquet" --output "$root/scene_store" \
