#!/bin/bash
#SBATCH --job-name=diag5c_verifyR
#SBATCH --time=04:00:00
#SBATCH --mem=24G
#SBATCH --cpus-per-task=8
#SBATCH --gpus-per-node=1
#SBATCH --partition=cip
#SBATCH --output=/home/z/Zekang.Zhang/logs/diag5c_verifyR_%j.out
#
...
#       --mem=24G --cpus-per-task=8 jobs/job_diag5c_verifyR.sh
set -o pipefail
...
SIZES=${SIZES:-500,1000,2500,5000,10000}
DELTA=${DELTA:-0.01}
CHUNK=${CHUNK:-24}
SEED=${SEED:-11}

date; nvidia-smi --query-gpu=name,memory.total --format=csv,noheader 2>/dev/null
stdbuf -oL -eL python -u scripts/diag5c_verifyR_snis.py \
  --n-gal "$NGAL" --n-nodes "$NODES" --sizes "$SIZES" --delta "$DELTA" \
