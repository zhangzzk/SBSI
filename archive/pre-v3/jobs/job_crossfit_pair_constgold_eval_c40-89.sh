#!/bin/bash
#SBATCH --job-name=seqpair_cg50_eval
...
LOOKUP=$RUN/pair/constgold_c40-89/lookup.feather
OUT=$ROOT/results/constgold_m_swap_v22_crossfit_pair_c40-89_16seed.json
...
python -u scripts/eval_m_swap_emulator_v22.py \
  ...
  --lookup "$LOOKUP" \
  --candidate-tag v22_crossfit_pair_optuna50_v3_c40-89 \
  ...
  --min-case 40 --max-case 90 \
  --output "$OUT"
test -s "$OUT"
echo CROSSFIT_PAIR_CONSTGOLD_EVAL_C40_89_DONE
date
