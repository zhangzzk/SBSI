...
#SBATCH --output=/home/z/Zekang.Zhang/logs/abv2sup_score_%A_%a.out
#SBATCH --error=/home/z/Zekang.Zhang/logs/abv2sup_score_%A_%a.err
set -euo pipefail
...
ORIGINAL_REFERENCE=$ROOT/results/anchorblend_g002_response_v22_c${HUNDRED}-${NEXT}.feather
COMPLEMENT_REFERENCE=$ROOT/results/anchorblend_g002_response_v2complement_c${HUNDRED}-${NEXT}.feather
...
for case in $(seq "$START" "$STOP"); do
  env SLURM_PROCID=0 python -u scripts/score_anchor_response_model.py \
    --base "$ORIGINAL_BASE" --reference "$ORIGINAL_REFERENCE" \
    --model-tag lsst_r_extnbr_indom_tuned --reg-file "$MODEL" \
    --expected-reg-sha256 "$MODEL_SHA" --candidate-label "$LABEL" \
    --output-dir "$ORIGINAL_PARTS" --case-offset "$case" --n-cases 1 --sign 0.02
  env SLURM_PROCID=0 python -u scripts/score_anchor_response_model.py \
    --base "$COMPLEMENT_BASE" --reference "$COMPLEMENT_REFERENCE" \
    --reference-baseline-column R_blend_lsst_r_extnbr_indom_tuned \
    --model-tag lsst_r_extnbr_indom_tuned --reg-file "$MODEL" \
    --expected-reg-sha256 "$MODEL_SHA" --candidate-label "$LABEL" \
    --output-dir "$COMPLEMENT_PARTS" --case-offset "$case" --n-cases 1 --sign 0.02
done
echo "ANCHOR_V2_SUPPLEMENT_SCORE_DONE cases=$START-$STOP"
