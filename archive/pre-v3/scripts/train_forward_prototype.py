#!/usr/bin/env python
"""EXPERIMENTAL feasibility gate: one shared-trunk forward model, two heads, four-term loss.

Trains ``sbs_shear.forward_model.SetConditionedForwardModel`` (a DeepSets scene trunk feeding
(a) a ConditionalMeanFlow over the measured shape whose explicit mean head carries the shape
response and (b) a Bernoulli detection head) with a MULTI-TERM loss that supervises the
SEPARABLE pieces against their OWN clean targets:

  L = L_NLL + lam_r * L_shape_resp + lam_d * L_BCE + lam_s * L_sel_resp

  L_NLL        -mean log p(measured_e1_m, measured_e2_m | scene)         [FLOW stream, detected]
  L_shape_resp per-(flux x size)-bin || R_model - R_self ||^2            [FLOW stream]
  L_BCE        BCE(detection_logit, detected)                            [DET stream, det+undet]
  L_sel_resp   per-(mag)-bin || dP/dgamma_model - b_true(mag) ||^2       [DET stream]

R_model / dP-model are induced by applying the analytic reduced-shear map S_{+/-delta} to the
INTRINSIC shapes (primary AND nearest neighbour), rebuilding the scene features, recomputing
the SAME trunk context, and taking a central secant (trace/2 convention, mirroring
scripts.train_measurement_model.epoch_response and scripts.eval_self_response_bins).

The feasibility question: can ONE trunk reproduce the shape self-response AND the detection
selection-response simultaneously, without the two trading off?  Trained here with two
single-head baselines (flow+shape-loss only; detection+sel-loss only) for the no-trade-off
comparison (V3).

This is a DIAGNOSTIC prototype.  It is ADDITIVE and is NEVER wired into the certified
parameter-free m = R_sim / (R_flow + R_blend) - 1.  See forward_model.py.
"""
...
SHAPE_TARGETS = ["measured_e1_m", "measured_e2_m"]   # FLOW g=0 leg measured shape (2D)
...
def load_stream(path, kind, max_case, train_case_max, max_rows_tr, max_rows_va, seed):
    ...
    with ipc.open_file(path) as reader:
        ...
        for bi in range(reader.num_record_batches):
            ...
            if kind == "flow":
                ...
                # require finite ONLY on the columns actually present: measured shape always;
                # delta_et* only if this is the pairs product (no both-legs filter otherwise).
                fcols = [c for c in ["measured_e1_m", "measured_e2_m", "delta_et1", "delta_et2"]
                         if c in b.columns]
...
def shifted_feature_frame(base, intr, gdir, delta):
    """Copy base, apply S_{delta*gdir} to BOTH the primary and the neighbour intrinsic shape,
    rebuild scene features, and add the two scene-summary columns.  delta=0 -> unshifted."""
    f = base.copy()
...
def precompute_stream(base, kind, primary_pre, nbr_std, delta, device,
                      ...
                      dist_edges=None, decorrelate=False,
...
def flow_loss(model, D, idx, sc, delta, bin_tgt, nbins, equal_weight=False):
    """Return (L_NLL, L_shape_resp) as differentiable tensors.  When D carries decorrelation
    weights ('w'), both the NLL and the per-bin response are weighted by them.  equal_weight
    weights each occupied (flux,size,blend) response cell EQUALLY (per-bin mean still uses the
    decorrelation weights) so the rare high-response cells (large size) are not under-fit."""
    ctx0 = model.context(D["p_0"][idx], D["n_0"][idx], D["mask"][idx])
    lp = model.log_prob_obs(D["tgt"][idx], ctx0)
    wi = D.get("w")
    if wi is not None:
        wi = wi[idx]
        ...
        nll = -lp.mean()
    r_i = flow_response_perobj(model, D, idx, sc, delta)
...
# ----------------------------------------------------------------------------------------------
@torch.no_grad()
def eval_flow(model, D, sc, delta, bin_tgt, nbins, bs, empty_set=False):
    model.eval()
    r_all, bin_all, nll_sum, n = [], [], 0.0, 0
    for idx in batches(D["_n"], bs, D["mask"].device, shuffle=False):
        ...
        nll_sum += float((-model.log_prob_obs(D["tgt"][idx], ctx0)).sum())
        n += len(idx)
        mu1p = model.mu(model.context(D["p_e1p"][idx], D["n_e1p"][idx], m))
        mu1m = model.mu(model.context(D["p_e1m"][idx], D["n_e1m"][idx], m))
        mu2p = model.mu(model.context(D["p_e2p"][idx], D["n_e2p"][idx], m))
...
def train_one(mode, Ftr, Fva, Dtr, Dva, sc, args, primary_dim, neighbor_dim,
    ...
    for ep in range(1, epochs + 1):
        ...
        for i in range(nsteps):
            ...
            if do_flow:
                nll, resp = flow_loss(model, Ftr, fb[i % len(fb)], sc, d, bin_tgt, nbins,
                                      equal_weight=args.flow_equal_weight)
                loss = loss + nll + lam_r * resp
            if do_det:
        ...
        # validation
        vnll = vshape = vbce = vsel = vR = vdP = np.nan
        sel_obj = 0.0
        if do_flow:
            ef = eval_flow(model, Fva, sc, d, bin_tgt, nbins, args.batch_size)
            vnll, vR = ef["nll"], ef["R_global"]
            vshape = float(np.nanmean((ef["R_bin"] - bin_tgt.cpu().numpy()) ** 2))
            sel_obj += vnll + lam_r * vshape
        if do_det:
        ...
        if ep % args.log_every == 0 or ep == 1 or ep == epochs:
            print(f"  ep{ep:03d} tot={tot / max(nsteps,1):.4f} | "
                  f"nll={vnll:.4f} shp_resp={vshape:.2e} <R>={vR:+.4f} | "
...
def main():
    ...
    ap.add_argument("--flow-catalogue",
                    default="/project/ls-gruen/users/zekang.zhang/lsst_sims_fs2_25876/"
                            "self_response_catalogue_train_cases0_99.feather")
    ...
    Ftr_df, Fva_df = load_stream(args.flow_catalogue, "flow", args.max_case, args.train_case_max,
                                 args.max_rows_flow, args.max_rows_val, args.seed)
    Dtr_df, Dva_df = load_stream(args.det_catalogue, "det", args.max_case, args.train_case_max,
                                 args.max_rows_det, args.max_rows_val, args.seed + 1)
    ...
    target_std = TargetStandardizer.fit(Ff0, SHAPE_TARGETS)
    ...
    Ftr = precompute_stream(Ftr_df, "flow", primary_pre, nbr_std, args.delta, device,
                            ...
                            dist_edges=dist_edges, decorrelate=args.decorrelate)
    ...
    common = dict(Ftr=Ftr, Fva=Fva, Dtr=Dtr, Dva=Dva, sc=sc, args=args,
                  primary_dim=primary_dim, neighbor_dim=neighbor_dim,
                  bin_tgt=bin_tgt, nbins=nbins, b_true=b_true, nmb=nmb, device=device)

    # ---- main JOINT model ----
    joint, joint_hist, _ = train_one("joint", lam_r=args.lam_r, lam_s=args.lam_s, lam_d=args.lam_d,
                                     epochs=args.epochs, tag="joint", **common)
    ...
    lines = []
    lines.append("SBSI unified-forward-model PROTOTYPE -- feasibility gate (EXPERIMENTAL, not certified)")
    ...
    lines.append(f"   <R_model> global = {ef['R_global']:+.4f}   target <R_self> = {rself_val_global:.4f}"
                 f"   rel.err = {(ef['R_global'] / rself_val_global - 1) * 100:+.2f}%")
    ...
    torch.save({
        ...
        "model_config": dict(target_dim=2, primary_dim=primary_dim, neighbor_dim=neighbor_dim,
        ...
        "shape_targets": SHAPE_TARGETS, "delta": args.delta,
        "flux_edges": flux_edges, "size_edges": size_edges, "mag_edges": mag_edges,
        "metadata": {"experimental": True, "not_certified": True,
                     "note": "prototype unified forward model; never wired into m=R_sim/(R_flow+R_blend)-1"},
    }, ckpt_path)
    ...
    print("FORWARD_PROTO_DONE", flush=True)
