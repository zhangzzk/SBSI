...
STEM=$(printf '%s/candidate_%02d' "$RUN" "$ID")
...
python -u scripts/train_v22_grouped_rscene_hparam_candidate.py \
  --source-cache "$SOURCE" --scene-cache "$SCENE" \
  --candidate-id "$ID" --output-model "$STEM.json" \
  --output-summary "$STEM.summary.json"
echo "V22_RSCENE_HPARAM_CANDIDATE_JOB_DONE id=$ID"
date
