...
def main():
    ...
    ap.add_argument(
        ...
        default=None,
        help=(
            "optional regression-model JSON override; selection cuts and target "
            "standardization still come from --tag metadata"
        ),
    )
    args = ap.parse_args()

    if args.reg_file is not None and not os.path.isfile(args.reg_file):
