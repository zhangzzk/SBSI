#!/bin/bash
#SBATCH --job-name=seqps_cg_lookup
...
MODEL_SUMMARY=$RUN/pair_scene/summary.json
OUTDIR=$RUN/pair_scene/constgold_c40-139
LOOKUP=$OUTDIR/lookup.feather
SUMMARY=$ROOT/results/v22_crossfit_pair_scene_constgold_lookup_c40-139.json
...
test -s "$LOOKUP"
test -s "$SUMMARY"
echo CROSSFIT_PAIR_SCENE_CONSTGOLD_LOOKUP_JOB_DONE
date
