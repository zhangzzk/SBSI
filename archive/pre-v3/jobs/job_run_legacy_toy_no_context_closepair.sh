...
NREAL=${TOY_NREAL:-400}
OUTDIR=${TOY_OUTPUT_DIR:-/project/ls-gruen/users/zekang.zhang/sbsi_caches/derisk/legacy_toy_no_context_exact_v1/closepair}
...
"$PY" -u scripts/run_legacy_toy_no_context.py \
  --suite closepair --config-id "$CONFIG_ID" --nreal "$NREAL" \
  --g 0.05 --sky 10 --stamp 96 \
  --output-draws "$DRAW" --output-json "$AUDIT"
test -s "$DRAW"
test -s "$AUDIT"
echo LEGACY_TOY_NO_CONTEXT_CLOSEPAIR_JOB_DONE
date
