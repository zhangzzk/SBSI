#!/bin/bash
# Shape production for the five V2-complement blocks, using the recorded V2 setup.
#SBATCH --job-name=lv2as_shape
...
#SBATCH --output=/home/z/Zekang.Zhang/logs/lv2as_shape_%A_%a.out
#SBATCH --error=/home/z/Zekang.Zhang/logs/lv2as_shape_%A_%a.err
set -euo pipefail
...
srun -n 100 --mpi=pmi2 python -u run_shape.py "$BASE" --case_start "$START" \
  --realizations d,0,1 --shear_case="$SHEAR" --stamp_size 48 --use_pos detect \
  --pixel_scale 0.2 --tile_name tile180.0_-0.5 --targets all --centering subpixel
...
echo "LEGACY_V2_ANCHOR_SHAPE_DONE cases=$START-$STOP shear=$SHEAR"
