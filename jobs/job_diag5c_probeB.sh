#!/bin/bash
#SBATCH --job-name=diag5c_probeB
#SBATCH --time=06:00:00
#SBATCH --mem=24G
#SBATCH --cpus-per-task=8
#SBATCH --gpus-per-node=1
#SBATCH --partition=cip
#SBATCH --output=/home/z/Zekang.Zhang/logs/diag5c_probeB_%j.out
#
...
#       --mem=24G --cpus-per-task=8 jobs/job_diag5c_probeB.sh
set -o pipefail
...
NGAL=${NGAL:-20000}
NODES=${NODES:-2000,6000,10000,20000}
GAMMA=${GAMMA:-0.05}
DELTA=${DELTA:-0.01}
CHUNK=${CHUNK:-24}
SEED=${SEED:-11}
...
stdbuf -oL -eL python -u scripts/diag5c_probeB_integrand.py \
  --n-gal "$NGAL" --n-nodes "$NODES" --gamma "$GAMMA" --delta "$DELTA" \
  --chunk "$CHUNK" --seed "$SEED" --n-draws "$DRAWS" --n-boot "$BOOT" ${EXTRA:-} 2>&1 \
