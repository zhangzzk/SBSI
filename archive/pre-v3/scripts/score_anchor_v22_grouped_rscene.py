#!/usr/bin/env python3
"""Score coherent-anchor pair manifests with a frozen grouped residual model.

This stage reads only latent/model inputs and the already deployed V2.2 pair
predictions.  It does not read coherent response truth.  One output shard is
written per rendered case for an embarrassingly parallel SLURM array.
"""
...
def block_base(case: int) -> Path:
    ...
    return Path(
        ...
        f"lsst_sims_fs2_25876_anchorblend_g002_c{start}-{stop}"
    )
...
def main() -> None:
    ...
    if summary["feature_names"] != CONDITIONAL_FEATURES:
        raise RuntimeError("final grouped model feature definition drifted")
    if summary["training"]["case_window"] != [40, 199]:
        raise RuntimeError("anchor scoring requires the frozen c40--199 final fit")
    model_path = Path(summary["model"])
    if sha256(model_path) != summary["model_sha256"]:
        raise RuntimeError("final grouped model hash differs from summary")
    booster = xgb.Booster({
        "device": "cpu",
    ...
    latent = pd.read_feather(
        catalogue_path,
        columns=[
            "index_input", "Re_input", "r_input", "sersic_n_input"
        ],
    ...
    scaled_frame = data_utils.rescale(raw_frame.copy(), **RESCALE)
    scaled = scaled_frame[MODEL_FEATURES].to_numpy(np.float32)
    ...
    features = conditional_matrix(
        scaled, raw, pair_prediction, scene_rows, count_rows, slice(None)
    )
    correction = physical_correction(
        booster, features, float(summary["target_scale"])
    )
    correction_sum = np.add.reduceat(correction, starts)
    corrected_sum = scene_sum.astype(np.float64) + correction_sum
    paired_anchor = anchor_index[starts]

    scored = pd.DataFrame({
        "input_index": paired_anchor,
        "n_pairs": counts.astype(np.int16),
        "R_blend_v22_replay": scene_sum.astype(np.float64),
        "grouped_pair_correction_sum": correction_sum,
        "R_blend_grouped": corrected_sum,
    })
    ...
    output.loc[no_pair, [
        "n_pairs", "R_blend_v22_replay", "grouped_pair_correction_sum",
        "R_blend_grouped",
    ]] = 0.0
    output["n_pairs"] = output.n_pairs.astype(np.int16)
    output.insert(0, "case", np.int16(args.case))
    numeric = output.drop(columns=["case", "input_index"]).to_numpy(float)
    if not np.isfinite(numeric).all():
        raise RuntimeError(f"case {args.case}: non-finite anchor score")

    temporary_feather = output_feather.with_name(
        f".{output_feather.stem}.{os.getpid()}.tmp.feather"
    )
    output.to_feather(temporary_feather)
    os.replace(temporary_feather, output_feather)
    payload = {
        "case": args.case,
        ...
        "pair_correction_std": float(correction.std(ddof=1)),
        "scene_correction_mean": float(correction_sum.mean()),
        "scene_correction_std": float(correction_sum.std(ddof=1)),
        ...
        "output_feather": str(output_feather),
        "coherent_response_truth_opened": False,
        "constgold_opened": False,
    }
    strict_json(output_json, payload)
    print(json.dumps(payload, indent=2, sort_keys=True))
    print("ANCHOR_V22_GROUPED_RSCENE_SCORE_DONE", flush=True)
