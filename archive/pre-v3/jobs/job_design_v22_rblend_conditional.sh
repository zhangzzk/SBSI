#!/bin/bash
#SBATCH --job-name=v22rbcond
#SBATCH --time=01:30:00
#SBATCH --mem=128G
#SBATCH --cpus-per-task=8
#SBATCH --partition=cluster
#SBATCH --constraint=x86-64-v3
#SBATCH --output=/home/z/Zekang.Zhang/logs/v22rbcond_%j.out
set -euo pipefail
...
COND=results/v22_rblend_bin_design_conditional8_target.npz
BASE=results/response_target_crowd_rblend_snc_c0-99_6x6x5_v22.npz
OUT=results/v22_rblend_bin_design_with_conditional.json
CSV=results/v22_rblend_bin_design_with_conditional.csv
for file in "$D/det_meas_crowd_g0.05_val_full.feather" \
            "$D/det_meas_crowd_conc_g0.0_train_full.feather" \
            "$MAIN_RESULTS/g0_lookup_c0-99.feather" "$BASE" "$DECILE"; do
  [ -f "$file" ] || { echo "MISSING $file"; exit 1; }
done
[ ! -e "$COND" ] && [ ! -e "$OUT" ] && [ ! -e "$CSV" ] || {
  echo "REFUSING overwrite conditional design outputs"; exit 1;
}

echo "### V2.2 CONDITIONAL R_BLEND BIN DESIGN — COUNTS ONLY, NO RETRAINING ###"; date
"$PY" -u scripts/compute_response_target_blend.py \
  ...
  --n-flux 6 --n-size 6 --n-crowd 8 --min-count 500 --max-case 99 \
...
"$PY" -u scripts/analyze_v22_rblend_bin_design.py \
  --decile-target "$DECILE" --baseline-target "$BASE" --conditional-target "$COND" \
  --flow-catalogue "$D/det_meas_crowd_conc_g0.0_train_full.feather" \
  --output "$OUT" --csv "$CSV"
echo V22_RBLEND_CONDITIONAL_DESIGN_DONE; date
