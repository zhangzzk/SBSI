...
#SBATCH --output=/home/z/Zekang.Zhang/logs/cgv30ns4_%A_%a.out
set -euo pipefail
...
MODEL=$((TASK / NCHUNK))
CHUNK=$((TASK % NCHUNK))
ARM_INDEX=$((MODEL / NSEED))
SEED_INDEX=$((MODEL % NSEED))
...
echo "### V3.0 FOUR-SEED CONSTGOLD arm=$ARM seed=$SEED cases=$MIN_CASE-$MAX_CASE ###"; date
"$PY" -u scripts/validate_constant_with_blend.py \
  ...
  --dump "$OUT"
echo "V30_NOSIZE_MORESEED_CONSTGOLD_DONE arm=$ARM seed=$SEED cases=$MIN_CASE-$MAX_CASE"; date
