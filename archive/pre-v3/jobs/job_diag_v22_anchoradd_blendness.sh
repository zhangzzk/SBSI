#!/bin/bash
#SBATCH --job-name=v22_addblend
#SBATCH --time=03:00:00
#SBATCH --mem=100G
#SBATCH --cpus-per-task=8
#SBATCH --partition=cluster
#SBATCH --constraint=x86-64-v3
#SBATCH --output=/home/z/Zekang.Zhang/logs/v22_addblend_%j.out
set -euo pipefail
...
OFFSET=$("$PY" -c 'import json,sys; print(json.load(open(sys.argv[1]))["additive_calibration"]["deployment_offset"])' "$CAL")
[ -n "${OFFSET:?}" ] || exit 1
"$PY" -u scripts/diag_v22_blendness.py \
  ...
  --mag-max 25.8 --re-min 0.5 --rblend-offset "$OFFSET"

echo V22_ANCHORADD_BLENDNESS_DONE; date
