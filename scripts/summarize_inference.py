#!/usr/bin/env python
"""Summarize paired inference arms with simulation cases as blocks."""
...
def load_inference_summary(path: str | Path) -> dict:
    ...
    implementation = payload.get(
        "pipeline_implementation_sha256", payload.get("implementation_sha256")
    )
    ...
    identity = {
        ...
        "pipeline_config_sha256": payload.get("pipeline_config_sha256"),
        "pipeline_resolved_config_sha256": payload.get(
            "pipeline_resolved_config_sha256"
        ),
        "likelihood_release": payload.get("likelihood_release"),
        "likelihood_config_sha256": payload.get("likelihood_config_sha256"),
        "likelihood_component_sha256": payload.get(
            "likelihood_component_sha256"
        ),
        "likelihood_implementation_sha256": payload.get(
...
# Historical result files and callers used the old function name.  Keeping the
# alias costs no separate implementation and lets the cleaned summarizer audit
# those immutable outputs.
load_recenter_summary = load_inference_summary
...
def main(argv=None):
    ...
    rows = [load_inference_summary(path) for path in args.results]
