...
#SBATCH --output=/home/z/Zekang.Zhang/logs/rbsumfid_%j.out
...
NPZ=$E/rblend_measured_allnbr_FIDdom_ap7g0.2_ladder.npz

echo "### SUMMED R_BLEND FIDUCIAL-DOMAIN job=$SLURM_JOB_ID ###"; date
python -u scripts/eval_rblend_gap_measured.py \
  ...
  --output "$NPZ" 2>&1 | grep -v --line-buffered "module command" || { echo RBSUMFID_FAILED; exit 1; }
