...
#SBATCH --output=/home/z/Zekang.Zhang/logs/lk_ho_%j.out

# STEP 5 of V2.1: per-(case,input_index) summed R_blend on constgold cases 40-139, from the V2.1
# emulator `lsst_r_extnbr_ho`, so the V2.1 flow's constgold m can be formed by a pure join.
#
...
mkdir -p results
echo "### blend_lookup 40-139, tag=lsst_r_extnbr_ho job=$SLURM_JOB_ID ###"; date
python -u scripts/build_blend_lookup.py --cases $(seq 40 139) --tag lsst_r_extnbr_ho \
  --output results/blend_lookup_extnbrho_c40-139.feather 2>&1 | grep -v --line-buffered "module command" \
  || { echo "LK_HO_FAILED (build)"; exit 1; }
if [ -f results/blend_lookup_extnbrho_c40-139.feather ]; then echo LK_HO_DONE;
else echo "LK_HO_FAILED (no output)"; exit 1; fi
date
