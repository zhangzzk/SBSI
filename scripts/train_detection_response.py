...
def main():
    ...
    ap.add_argument("--reference-model", default=None,
                    help="optional existing classifier to copy feature set + arch from; if omitted, "
                         "use --feature-set + --hidden/--n-layers directly (no base ckpt needed)")
    ap.add_argument("--feature-set", default="g0_shearfree",
                    help="feature-set name from sbs_shear.selection_model.SELECTION_FEATURE_SETS")
    ...
    if args.reference_model:
        ...
        from sbs_shear.selection_model import SELECTION_FEATURE_SETS
        feature_names = list(SELECTION_FEATURE_SETS[args.feature_set])
        hidden, n_layers, act = args.hidden_dim, args.n_layers, args.activation
    print(f"feature set '{args.feature_set}' ({len(feature_names)}): {feature_names}")
