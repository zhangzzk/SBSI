...
#SBATCH --output=/home/z/Zekang.Zhang/logs/ab_paircap_%A_%a.out
#SBATCH --error=/home/z/Zekang.Zhang/logs/ab_paircap_%A_%a.err
set -euo pipefail
...
for case in $(seq "$START" "$END"); do
  env SLURM_PROCID=0 python -u scripts/rescore_anchor_independent_pair_cap.py \
    ...
    --case-offset "$case" --n-cases 1 --pair-k 64
done
echo "ANCHORBLEND_INDEPENDENT_PAIRCAP_PART_DONE cases=$START-$END"
date
