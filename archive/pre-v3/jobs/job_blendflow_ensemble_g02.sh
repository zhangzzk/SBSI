...
#SBATCH --array=0-15
#SBATCH --output=/home/z/Zekang.Zhang/logs/bfens02_%A_%a.out
#SBATCH --error=/home/z/Zekang.Zhang/logs/bfens02_%A_%a.err

# ==== g = 0.2 VARIANT (WORKLOG 2026-08-03b) ====
# STAGE 4/4 of the g=0.2 retrain. IDENTICAL to job_blendflow_ensemble.sh except for the pair set and
# the output tag -- every hyperparameter is deliberately frozen at the round-2 configuration so that
# the ONLY difference from the g=0.05 ensemble is the shear amplitude the labels were measured at.
# That is what makes the two ensembles comparable, and it is the whole point of the exercise.
# Checkpoints land as blendflow_ens_g02_s<seed>.pt, distinct from the g=0.05 blendflow_ens_s<seed>.pt,
# so nothing is overwritten and both ensembles remain available side by side.
# See the curvature measurement (job 15485364): if c g^2 at g=0.2 is a large fraction of R_0, the
# labels this trains on carry that bias and the resulting `m` must be read with it.
...
PAIRSET=$CACHE/blend_pairset_ap7_g02.feather
...
if [ ! -f "$PAIRSET" ]; then
  echo "MISSING $PAIRSET -- run jobs/job_blend_pairset_g02.sh first"; exit 1
fi
