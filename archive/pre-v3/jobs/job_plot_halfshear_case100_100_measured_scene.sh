...
OUTPUT=$ROOT/results/halfshear_case100_100_residual_vs_measured_scene_c100-199
...
python -u scripts/plot_halfshear_case100_100_residual_vs_measured_scene.py \
  --source-cache "$SOURCE" \
  --full-neighbour-cache "$FULL" \
  --stack-summary "$RUN/summary.json" \
  --output-stem "$OUTPUT"
echo "PLOT_HALFSHEAR_CASE100_100_MEASURED_SCENE_JOB_DONE"
date
