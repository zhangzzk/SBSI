...
#SBATCH --output=/home/z/Zekang.Zhang/logs/bfallp02_%j.out
#SBATCH --error=/home/z/Zekang.Zhang/logs/bfallp02_%j.err
...
CKPTS=""
for s in $SEEDS; do
  f=$CACHE/blendflow_ens_g02_s${s}.pt
  [ -f "$f" ] || { echo "MISSING $f"; exit 1; }
  CKPTS="$CKPTS $f"
done
LOOKUP=$CACHE/blend_lookup_ens16g02_allpairs7_c40-139.feather
...
python -u scripts/build_blend_lookup_both.py --cases 40 41 --checkpoint $CKPTS \
    ...
    --output "$CACHE/blend_lookup_ens16g02_allpairs7_SMOKE.feather" || exit 1
