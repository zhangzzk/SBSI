...
from scripts.diag_v22_blendness import report_paired_total_m_change, report_total_m
...
FLUX_COLUMNS = [
    ...
    ("top-two bright rank", "logflux_top2_0_10"),
    ("third+ bright rank", "logflux_thirdplus_0_10"),
]
SOURCE_FLUX_COLUMNS = [column for _, column in FLUX_COLUMNS if column != "logflux_top2_0_10"]
ZERO_POINT = 30.0


def flux_quantile_table(title, key, domain, blocks, axis_label="log10(1+Fn/Fp)"):
    ...
    return Table(title, axis_label, labels, ids, len(labels), *blocks), edges
...
def main() -> None:
    ...
    ap.add_argument(
        "--flux-mode", choices=("relative", "absolute"), default="relative",
        help="bin on Fn/Fp (legacy) or intrinsic absolute Fn in simulation count units",
    )
    args = ap.parse_args()
    t0 = time.time()
    ...
    dumps = [candidate_by_seed[seed] for seed in seeds]
    ...
    if args.reference_dump_glob:
        ...
        reference_dumps = [reference_by_seed[seed] for seed in seeds]
    ...
    lookup = pf.read_table(
        args.flux_lookup,
        columns=["case", "input_index", *SOURCE_FLUX_COLUMNS],
    ).to_pandas()
    ...
    all_ratio = np.power(10.0, keyed["logflux_all_0_10"].to_numpy(float)) - 1.0
    third_ratio = np.power(10.0, keyed["logflux_thirdplus_0_10"].to_numpy(float)) - 1.0
    keyed["logflux_top2_0_10"] = np.log10(1.0 + np.maximum(all_ratio - third_ratio, 0.0))
    axis_label = "log10(1+Fn/Fp)"
    if args.flux_mode == "absolute":
        # The lookup stores Fn/Fp. Reconstruct Fn exactly using the primary's intrinsic
        # magnitude and the simulation zero point. This removes primary brightness from
        # the scene coordinate; log1p only compresses its dynamic range.
        primary_flux = np.power(10.0, -0.4 * (mag - ZERO_POINT))
        for _, column in FLUX_COLUMNS:
            ratio = np.power(10.0, keyed[column].to_numpy(float)) - 1.0
            keyed[column] = np.log10(1.0 + ratio * primary_flux)
        axis_label = "log10(1+Fn)"
    ...
    for title, column in FLUX_COLUMNS:
        table, edges = flux_quantile_table(
            ...
            keyed[column].to_numpy(float), domain, blocks, axis_label=axis_label,
        )
        tables.append(table)
    ...
    if reference_dumps is not None:
        reference_tables = []
        for title, column in FLUX_COLUMNS:
            table, _ = flux_quantile_table(
                f"reference by intrinsic neighbour flux: {title}",
                keyed[column].to_numpy(float), domain, blocks, axis_label=axis_label,
            )
    ...
    if reference_dumps is not None:
        for path in reference_dumps:
            dump = pf.read_table(path, columns=["r_sim", "R_flow", "R_blend"])
            rs = dump["r_sim"].to_numpy(zero_copy_only=False).astype(float)
            rb = dump["R_blend"].to_numpy(zero_copy_only=False).astype(float)
            if not np.array_equal(rs, first_rsim) or not np.array_equal(rb, first_rb):
                raise RuntimeError(f"reference R_sim/R_blend differs from candidate: {path}")
            values = (rs, dump["R_flow"].to_numpy(zero_copy_only=False).astype(float), rb)
    ...
    print(f"Flux mode: {args.flux_mode}; axis={axis_label}; zero point={ZERO_POINT:.1f}.")
