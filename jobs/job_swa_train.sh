#!/bin/bash
#SBATCH --job-name=swa_train
#SBATCH --time=05:00:00
#SBATCH --mem=36G
#SBATCH --cpus-per-task=8
#SBATCH --gres=gpu:a40-24gb:1
#SBATCH --partition=cip
#SBATCH --output=/home/z/Zekang.Zhang/logs/swa_train_%j.out
...
echo "### SWA_TRAIN(cip) job=$SLURM_JOB_ID seeds='$SEEDS' FS=$FS TAG=$TAG LAM=$LAM WD=$WD RESP=$RESP ###"
nvidia-smi -L; date
...
for SEED in $SEEDS; do
  ...
  python -u scripts/train_measurement_model_swa.py \
    ...
    --response-target-npz "$RESP" || { echo "SWA_TRAIN FAILED seed=$SEED"; exit 1; }
  echo "TRAIN_DONE seed=$SEED MODEL=$OUT"; date
done
echo "SWA_TRAIN_ALL_DONE job=$SLURM_JOB_ID seeds='$SEEDS' TAG=$TAG"; date
