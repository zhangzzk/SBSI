...
def load_model(path, device):
    ...
    model = SetConditionedForwardModel(
        ...
        activation="silu", pooling="sum", base_flow="affine",
        shape_skip_dim=mc.get("shape_skip_dim", 0),      # worktree shape-skip fix (0 for baseline ckpts)
        shape_skip_idx=mc.get("shape_skip_idx"),
        shape_skip_hidden=mc.get("shape_skip_hidden", 0)).to(device)
