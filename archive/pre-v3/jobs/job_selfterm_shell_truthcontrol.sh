...
OUT=$ROOT/results/selfterm_shell_step_truthcontrol_c40-139_s16.json
...
cd "$ROOT"
python -u scripts/diag_selfterm_shell_step.py \
  ...
  --distances closest_pair_distance dominant_distance \
  --bin-width 0.25 --range 0.5 10.0 \
  --columns self_term S_h R_flow_h \
  --output "$OUT"
echo SELFTERM_CTRL_JOB_DONE; date
