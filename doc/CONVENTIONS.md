...
For the configured additive likelihood, `R_blend` is a fixed atom-aligned
external response contribution averaged over the same selected population.
For V3.6-like, it is the response contribution induced by the output-conditioned
disk-velocity predictor and Möbius transport. The disk parameter itself must
not be interpreted as an additive response or copied into an additive cache.

The retained development identity is `V3.6-like`: original common-descent
single flow, physical-moment disk emulator, and single 17-input
smooth-crowding classifier. See [the model contract](JOINT_CALIBRATION.md#goal-and-retained-population).
`V3.5-like` remains the distinct historical epoch154/trial9/three-classifier
preset. Neither name changes the numerical inference release.
...
Measurement provenance must also state stamp size, pixel scale, galaxy
Jacobian centering (geometric stamp centre or explicit subpixel source
centre), and optimizer random-stream handling. Matching galaxy identities
and image noise does not make catalogues with different centering conventions
interchangeable. A step-size comparison requires the same measurement
definition in every leg; unresolved historical centering is not an amplitude
effect. See the [measurement centering investigation](../archive/research-2026-09-20/doc/JOINT_CALIBRATION.md#measurement-centering-investigation).
...
The owner retained the existing development model as V3.6-like and stopped
the investigation on 2026-09-20. This decision does not relabel reused
endpoints or the incomplete new scene set as fresh acceptance evidence; see
[the stopping record](JOINT_CALIBRATION.md#independent-validation-stopped).
