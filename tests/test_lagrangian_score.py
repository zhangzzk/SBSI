...
from sbsi.lagrangian_score import (
    curve_derivatives,
...
@pytest.fixture(scope="module")
def evaluated_catalogues(bank):
    """Cache the two expensive A.7 catalogue evaluations shared by three tests."""

    z, log_prior, w = bank
    s_sel, i_sel = population_curve(lambda t: norm.sf((CUT - z - t) / SIGMA), weights=w)
    catalogues = {}
    for truth in (0.0, 0.05):
        y = kept_sample(truth)
        p0, d1, d2 = phi_curves(y, z)
        s, info = score_and_information(p0, d1, d2, log_prior=log_prior)
        catalogues[truth] = (y, s, info)
    return catalogues, s_sel, i_sel


def test_estimator_under_a_cut_matches_the_closed_form(evaluated_catalogues):
    ...
    """
    truth = 0.05
    catalogues, s_sel, i_sel = evaluated_catalogues
    y, s, info = catalogues[truth]
    ...
    assert louis == pytest.approx(shear_estimate_louis(s_ref, i_ref, s_sel_ref, i_sel_ref), rel=1e-5)
...
def test_centring_is_not_optional_in_either_moment(evaluated_catalogues):
    """Dropping `<s>_sel`, or dropping `I_sel` alone, are both O(1) errors."""
    truth = 0.05
    catalogues, s_sel, i_sel = evaluated_catalogues
    _, s, info = catalogues[truth]
...
def test_denominator_consistency_and_the_5_9b_drift(evaluated_catalogues):
    ...
    truth = 0.05
    _, s, info = catalogues[truth]
    measured = shear_estimate_bartlett(s, s_sel) / truth - shear_estimate_louis(s, info, s_sel, i_sel) / truth
