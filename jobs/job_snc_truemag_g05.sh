#!/bin/bash
#SBATCH --job-name=snc_tm_g05
#SBATCH --time=05:00:00
#SBATCH --mem=32G
#SBATCH --cpus-per-task=8
#SBATCH --gpus-per-node=1
#SBATCH --partition=cip
#SBATCH --output=/home/z/Zekang.Zhang/logs/snc_tm_g05_%j.out
...
CAT=/project/ls-gruen/users/zekang.zhang/sbsi_catalogues/det_meas_ngmix_np7_g0.05_val.feather
run () {  # $1 = true-mag-max label ("none" or a number)
  local tm="$1"; local arg=""
  [ "$tm" != "none" ] && arg="--true-mag-max $tm"
  echo "### NP7 g=0.05 SNC  true-mag-max=$tm ###"
  python -u scripts/validate_allpairs_response.py --measurement-model "$M" --catalogue "$CAT" \
    --nominal-g 0.05 --max-rows 0 --n-samples 64 --snc-lookup "$LK" $arg 2>&1 \
