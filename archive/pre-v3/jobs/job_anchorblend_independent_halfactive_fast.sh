...
#SBATCH --output=/home/z/Zekang.Zhang/logs/ab_halffast_%A_%a.out
#SBATCH --error=/home/z/Zekang.Zhang/logs/ab_halffast_%A_%a.err
set -euo pipefail
...
for case in $(seq "$START" "$END"); do
  env -u SLURM_PROCID python -u scripts/measure_anchor_independent_fast.py \
    --base "$BASE" --output-dir "$PARTS" --active-only \
    --case-offset "$case" --n-cases 1
done
echo "ANCHORBLEND_INDEPENDENT_HALF_ACTIVE_FAST_PART_DONE cases=$START-$END"
date
