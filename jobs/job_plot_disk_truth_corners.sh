#!/bin/bash
#SBATCH --job-name=sbsi-truth-corners
...
#SBATCH --output=logs/v36_truth_corners_%j.out
set -euo pipefail
...
PYTHONPYCACHEPREFIX=$(mktemp -d /tmp/sbsi_truth_corner_pycache.XXXXXX)
...
"$PYTHON" /home/z/Zekang.Zhang/.agents/skills/get-available-resources/scripts/detect_resources.py \
  --output "logs/v36_truth_corner_resources_${SLURM_JOB_ID}.json"
"$PYTHON" -m pytest tests/test_disk_truth_corners.py tests/test_disk_faint_corners.py -q
"$PYTHON" scripts/plot_disk_truth_corners.py --run "$RUN_ROOT" \
  --output "/home/z/Zekang.Zhang/SBSI/doc/figures/v36_truth_corners_${SLURM_JOB_ID}"
