#!/bin/bash
#SBATCH --job-name=abg002x_shape
#SBATCH --time=02:00:00
# Completed 100-rank blocks use about 42--55 GB total; keep >2x headroom
# while allowing the job onto the 256 GB cluster nodes.
#SBATCH --mem=128G
...
srun -n 100 --mpi=pmi2 python -u run_shape.py "$BASE" \
  --case_start "$AB_START" --realizations d,0,1 --shear_case="$SHEAR" \
...
echo "ANCHORBLEND_G002_EXT_SHAPE_DONE cases=${AB_START}-${AB_STOP} shear=$SHEAR"
date
