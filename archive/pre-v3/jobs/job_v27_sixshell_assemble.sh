#!/bin/bash
#SBATCH --job-name=v27sh_asm
#SBATCH --time=04:00:00
#SBATCH --mem=180G
#SBATCH --cpus-per-task=16
#SBATCH --partition=cluster
#SBATCH --constraint=x86-64-v3
#SBATCH --output=/home/z/Zekang.Zhang/logs/v27sh_asm_%j.out
set -euo pipefail
...
HALF=$C/scene_shells_hs_c0-199.feather
CONST=$C/scene_shells_const_c40-139.feather
[ -e "$HALF" ] || "$PY" -u scripts/compact_scene_feature_shards.py \
  --inputs "$C/parts/shell_features_hs_v27_c*.feather" \
  --manifest "$C/scene_anchor_manifest_hs_c0-199.feather" --output "$HALF"
[ -e "$CONST" ] || "$PY" -u scripts/compact_scene_feature_shards.py \
  --inputs "$C/parts/shell_features_const_v27_c*.feather" \
...
TRAIN=$C/det_meas_sixshell_v27_g0.0_train_v22domain.feather
TARGET=$C/det_meas_sixshell_v27_g0.05_val_v22domain.feather
BASE=$C/halfshear_sixshell_v27_base_c40-199_v22domain.feather
...
OUT=$C/response_target_v27_sixshell.joblib
[ -e "$OUT" ] || "$PY" -u scripts/build_scene_response_target_v27.py \
  --catalogue "$TARGET" --snc-lookup "$SNC" --arm sixshell --output "$OUT"
echo V27_SIXSHELL_ASSEMBLE_DONE; date
