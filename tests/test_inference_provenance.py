...
MODULE = load_script_module("run_inference.py")
...
def test_pipeline_and_likelihood_implementation_identities_are_separate():
    pipeline = MODULE._pipeline_implementation_hashes()
    likelihood = MODULE._likelihood_implementation_hashes()
    assert "scripts/run_inference.py" in pipeline
    assert "configs/inference.json" in pipeline
    assert "scripts/run_inference.py" not in likelihood
    assert "configs/inference.json" not in likelihood
    assert "configs/likelihood.json" in likelihood
