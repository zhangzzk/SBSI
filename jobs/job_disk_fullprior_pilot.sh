#!/bin/bash
#SBATCH --job-name=sbsi-v36-24m-pilot
#SBATCH --partition=inter,cip
#SBATCH --gres=gpu:a40:1
#SBATCH --cpus-per-task=4
#SBATCH --mem=96G
#SBATCH --time=00:45:00
#SBATCH --output=logs/v36_fullprior_pilot_%j.out
set -euo pipefail
...
"$PYTHON" /home/z/Zekang.Zhang/.agents/skills/get-available-resources/scripts/detect_resources.py \
  --output "logs/v36_fullprior_resources_${SLURM_JOB_ID}.json"
"$PYTHON" scripts/run_disk_inference.py run --subset "$RUN_ROOT/prior_subset24m/manifest.json" \
  ...
  --count 256 --object-chunk 16 --compile-flow --output "$RUN_ROOT/fullprior_pilot_${SLURM_JOB_ID}"
