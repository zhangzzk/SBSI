...
#SBATCH --output=/home/z/Zekang.Zhang/logs/flow_v27_%A_%a.out
set -euo pipefail
...
ARMS=(sixshell sixshell purity purity)
SEEDS=(501 502 501 502)
ARM=${ARMS[$TASK]}; SEED=${SEEDS[$TASK]}
if [ "$ARM" = sixshell ]; then
  FEATURE_SET=g0_meas_crowd_conc_szfl_noz_sixshell
else
  FEATURE_SET=g0_meas_crowd_conc_szfl_noz_purity
fi
...
CAT=$C/det_meas_scenev27_g0.0_train_v22domain.feather
TARGET=$C/response_target_v27_${ARM}.joblib
COUP=/home/z/Zekang.Zhang/SBSI/results/response_target_theta_coupling_rblend_c0-99_6x9x5.npz
TAG=ablate_s2c_lt500_v27_${ARM}
...
echo "### V2.7 arm=$ARM seed=$SEED ###"; nvidia-smi -L; date
...
echo V27_FLOW_DONE arm=$ARM seed=$SEED; date
