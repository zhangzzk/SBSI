...
#SBATCH --output=/home/z/Zekang.Zhang/logs/abfr_shape_%A_%a.out
#SBATCH --error=/home/z/Zekang.Zhang/logs/abfr_shape_%A_%a.err
set -euo pipefail
...
if [ "$SLURM_ARRAY_TASK_ID" -eq 0 ]; then SHEAR=0.05; else SHEAR=-0.05; fi
cd /home/z/Zekang.Zhang/blendemu/scripts
srun -n 100 --mpi=pmi2 python -u run_shape.py "$BASE" \
  --case_start 300 --realizations d,0,1 --shear_case="$SHEAR" \
...
echo "ANCHORBLEND_FRESH_SHAPE_DONE shear=$SHEAR"
date
