#!/bin/bash
#SBATCH --job-name=fwd_proto99sz10cg
...
#SBATCH --output=/home/z/Zekang.Zhang/logs/fwd_proto99sz10cg_%j.out
# TRACK 1 iter #5: metric-consistent constgold target at FINER size (6x10x5) to close the within-cell
# size gradient residual left by sz6cg. --flow-equal-weight, det unchanged. FIREWALL: a-priori, OOS.
set -e
...
TARGET=results/response_target_constgold_c0-99_6x10x5.npz
SEEDS=${SEEDS:-421 422 423}
echo "### FWD_PROTO99SZ10CG job=$SLURM_JOB_ID node=$SLURMD_NODENAME seeds='$SEEDS' ###"; nvidia-smi -L; date
...
for S in $SEEDS; do
  ...
  stdbuf -oL -eL python -B -u scripts/train_forward_prototype.py \
    ...
    --outdir "$OUTDIR" --tag "proto_c0-99_sz10cg_seed${S}" \
    --max-case 100 --train-case-max 80 \
    --max-rows-flow 1200000 --max-rows-det 1200000 --max-rows-val 500000 \
    --n-flux 6 --n-size 10 --delta 0.05 --lam-r 50 --lam-s 50 --lam-d 1 \
    ...
    || { echo "FWD_PROTO99SZ10CG FAILED seed=$S"; exit 1; }
  echo "SEED_DONE $S"; date
done
echo "### FWD_PROTO99SZ10CG_DONE job=$SLURM_JOB_ID ###"; date
